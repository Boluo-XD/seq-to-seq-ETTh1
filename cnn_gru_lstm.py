...
parser.add_argument('--horizon', type=int, default=96, help='number of measurements ahead to predict')
...
parser.add_argument('--num-epochs', type=int, default=1500, help='max num of epochs')
...
def predict(symbol, train_iter, val_iter, test_iter, data_names, label_names):
    ...
    # 加载模型参数
    params_file = "../models/electricity_model-1500.params"  # 参数文件的路径
    ...
    for i in range(len(test_pred)):
        ...
        pre_results.append(test_pred[i][7])
        real_results.append(test_label[i][7])
    ...
    # 绘制预测值曲线，使用蓝色实线
    plt.plot(pre_results, linewidth=2, label='Prediction')
 
    # 绘制真实值曲线，使用红色虚线
    plt.plot(real_results, linewidth=2, label='GroundTruth')
